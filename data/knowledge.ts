...
export const knowledge: KnowledgeEntry[] = [
  ...
  // ─── Current Role (Compass.uol) ──────────────────────────────────────────────
  {
    ...
    replies: {
      en: [
        ...
        "My current project is a multi-tenant sports platform with a trainer mobile app and an admin backoffice. The backend is a cluster of NestJS services on AWS, with PostgreSQL per service, Docker for containerisation, and Kafka for some of the event-driven flows. I joined as a senior engineer and have progressively taken on more architectural ownership — the kind of role I want to continue in.",
      ],
      no: [
  ...
  // ─── Databases ───────────────────────────────────────────────────────────────
  {
    ...
    replies: {
      en: [
        "PostgreSQL is my primary database — I've been using it professionally for years and have a solid handle on indexing strategies, query optimisation, and schema design for both OLTP and analytical queries. I've used Redis extensively for caching, session storage, and as a lightweight message broker. I've also worked with MySQL and Oracle DB (legacy ERP) and done meaningful performance work: identifying slow queries, rewriting them, adding targeted indexes, and in some cases redesigning schemas to avoid N+1 patterns.",
        "My database philosophy: schema design is one of the most important and least reversible decisions in a project. I prefer explicit migrations over auto-generated ones, and I take query performance seriously from day one — not as an afterthought. At both Velit and Compass.uol I've handled performance issues that were blocking features in production, which tends to sharpen your instincts quickly.",
      ],
      no: [
