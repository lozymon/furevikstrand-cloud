...
- [-] **A11y audit with real screen-reader testing** (NVDA/VoiceOver) — dropped. Blocker is hardware (dev machine is Linux; Orca isn't representative of what real users run), and marginal value over the axe-clean + ARIA work already shipped in P1 ("Chat surface ARIA wired", "Color contrast audited", "Skip-to-chat link") is low for a single-developer portfolio. Cheapest unblock if ever revisited: 15-min screen-share with someone running VoiceOver (`Cmd + F5`) on the deployed site.
- [-] **PWA / offline shell** — dropped for now. The PLAN entry undersold the cost: the keyword matcher currently runs server-side in `/api/chat` (`lib/chat.ts` + `data/knowledge/*.ts`), so offline fallback isn't just SW plumbing — it requires shipping the matcher + ~30-50 KB of knowledge data to the client as a fallback when `fetch('/api/chat')` rejects. Closer to 1.5 days than 1. Practical offline value is near-zero for a portfolio; the only real win is the "demonstrable PWA artifact" narrative, which isn't a current goal. Revisit if PWA experience becomes a thing worth showcasing.
