      ...
      <div className="max-w-[820px] mx-auto px-4 sm:px-6 lg:px-11 py-6 sm:py-10">
        ...
        <header className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-4 sm:gap-6 pb-[22px] border-b border-[#252535] mb-[26px]">
          ...
          <div className="flex flex-col gap-[5px] items-start text-left sm:items-end sm:text-right">
            ...
            <ContactItem
              label={locale === 'no' ? 'Sted' : locale === 'pt' ? 'Local' : 'Location'}
              value={
                locale === 'no'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_245px] gap-7 items-start">
          ...
          <div>
            ...
            <section className="mb-[22px]">
              <SectionTitle>
                {'// '}
                {locale === 'no' ? 'Om meg' : locale === 'pt' ? 'Sobre' : 'About'}
              </SectionTitle>
            ...
            <section className="mb-[22px]">
              <SectionTitle>
                {'// '}
                {locale === 'no' ? 'Erfaring' : locale === 'pt' ? 'Experiência' : 'Experience'}
              </SectionTitle>
              {expGroups.map((group) => (
                <div
                  key={group.company}
                  className="mb-4 pl-3 border-l-2 border-[#252535] relative last:mb-0"
                >
                  <span className="absolute -left-[5px] top-1 w-[7px] h-[7px] rounded-full bg-[#a78bfa] border-2 border-[#0d0d10]" />
                  <div className="flex justify-between items-baseline gap-2 mb-0.5">
                    <span className="text-[11.5px] font-semibold text-[#e2e2f0]">
                      {group.company}
                    </span>
                    <span className="text-[9px] text-[#8888a8] whitespace-nowrap shrink-0">
                      {group.overallPeriod}
                    </span>
                  </div>
                  ...
                  {group.entries.map((job) => (
                    <div key={job.id} className="mb-3 last:mb-0">
                      ...
                      <p className="text-[10px] text-[#8888a8] leading-[1.7] mb-1.5">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {job.tech.map((t) => (
                          <span
                            key={t}
                            className="text-[9px] px-[7px] py-0.5 bg-[#1e1e2e] border border-[#252535] rounded-[3px] text-[#38bdf8]"
                          >
                            {t}
                          </span>
                        ))}
            ...
            <section className="mb-[22px]">
              ...
              {testimonials.slice(0, 3).map((t) => (
                <div
                  key={t.name}
                  className="mb-[10px] p-[9px_11px] bg-[#161620] border border-[#252535] rounded-[6px] last:mb-0"
                >
                  <p className="text-[10px] text-[#8888a8] leading-[1.7] mb-2 italic">
            ...
            <section className="mb-[22px]">
              ...
              {highlighted.map((p) => (
                <div
                  key={p.id}
                  className="mb-[9px] p-[9px_11px] bg-[#161620] border border-[#252535] rounded-[6px] last:mb-0"
                >
                  <div className="flex justify-between items-baseline mb-1">
                    <span className="text-[11px] font-semibold text-[#a78bfa]">{p.name}</span>
                    {p.repo && (
                      <span className="text-[9px] text-[#8888a8]">
                        {p.repo.replace('https://', '')}
                      </span>
                    )}
                  </div>
                  <p className="text-[10px] text-[#8888a8] leading-relaxed mb-1.5">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-[7px] mb-[18px]">
              <StatBox
                ...
                label={locale === 'no' ? 'År' : locale === 'pt' ? 'Anos' : 'Years'}
              />
              <StatBox
                value={String(profile.stats.projects)}
                label={locale === 'no' ? 'Prosjekter' : locale === 'pt' ? 'Projetos' : 'Projects'}
              />
              <StatBox
                value={String(profile.stats.languages)}
                label={locale === 'no' ? 'Språk' : locale === 'pt' ? 'Idiomas' : 'Languages'}
              />
            </div>
            ...
            <section className="mb-[22px]">
              <SectionTitle>
                {'// '}
                {locale === 'no' ? 'Ferdigheter' : locale === 'pt' ? 'Habilidades' : 'Skills'}
              </SectionTitle>
              <SkillGroup
                label={locale === 'no' ? 'Topp ferdigheter' : 'Top Skills'}
                items={['NestJS', 'Microservices', 'TypeScript', 'AWS', 'AI / LLMs']}
              />
              <SkillGroup label="Frontend" items={stack.frontend} />
              <SkillGroup label="Backend" items={stack.backend} />
              <SkillGroup
                label={locale === 'no' ? 'Databaser' : locale === 'pt' ? 'Bancos' : 'Databases'}
            ...
            <section className="mb-[22px]">
              ...
              {certifications.slice(0, 5).map((cert) => (
                <div
                  key={cert.name}
                  className="py-1 border-b border-[#252535] last:border-0 text-[9px] text-[#8888a8] leading-relaxed"
                >
                  {cert.name}
                </div>
              ))}
            ...
            <section className="mb-[22px]">
              ...
              <InfoRow
                k={locale === 'no' ? 'Lokasjon' : locale === 'pt' ? 'Localização' : 'Location'}
                v="Natal, Brazil"
              />
              <InfoRow
                k={locale === 'no' ? 'Relokasjon' : locale === 'pt' ? 'Relocação' : 'Relocation'}
                v={
                  locale === 'no'
                    ? 'Åpen for Norge'
                    : locale === 'pt'
              ...
              <InfoRow
                k={
                  locale === 'no' ? 'Arbeidstype' : locale === 'pt' ? 'Tipo trabalho' : 'Work type'
                }
                v="Remote · On-site"
              />
              <InfoRow
                k={locale === 'no' ? 'Rolletype' : locale === 'pt' ? 'Tipo de cargo' : 'Role type'}
                v="Senior · Tech Lead"
              />
        ...
        <footer className="mt-6 pt-3 border-t border-[#252535] flex justify-between items-center text-[9px] text-[#8888a8]">
          <span>
            <span className="text-[#a78bfa]">Kim Andrè Furevikstrand</span> · {profile.email} ·
            furevikstrand.cloud
          </span>
...
function ContactItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center gap-1.5 max-w-full">
      <span className="text-[9px] uppercase tracking-[0.08em] text-[#8888a8] shrink-0">
        {label}
      </span>
    ...
    <div className="mb-[10px] last:mb-0">
      ...
      <div className="flex flex-wrap gap-1">
        {items.map((item) => (
          <span
            key={item}
            className="text-[9px] px-[7px] py-0.5 bg-[#1e1e2e] border border-[#252535] rounded-[3px] text-[#e2e2f0]"
          >
            {item}
          </span>
        ))}
