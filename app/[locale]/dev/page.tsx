...
function renderTerminalText(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return (
        <span key={i} className="text-[#90ff90] font-bold">
          {part.slice(2, -2)}
        </span>
      )
    }
    if (part.startsWith('`') && part.endsWith('`')) {
      return (
        <span key={i} className="text-[#28c840] bg-[#0a2a0a] px-1 rounded font-mono text-xs">
          {part.slice(1, -1)}
        </span>
      )
    }
    if (part.startsWith('*') && part.endsWith('*')) {
      return (
        <span key={i} className="text-[#4adb4a]">
          {part.slice(1, -1)}
        </span>
      )
    }
...
function storageKey(locale: string) {
  return `dev_chat_history_${locale}`
}
  ...
  const submitText = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        setLines((prev) => [
          ...prev,
          { id, type: 'ai', text: 'Something went wrong. Please try again.', source: 'fallback' },
        ])
      }
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 h-10 border-b border-[#0a2a0a] bg-[#010801] shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="hidden sm:inline text-[#1a6b1a] text-xs ml-2">
            kim@furevikstrand.cloud — bash
          </span>
          <span className="hidden sm:inline text-[#1a4a1a] text-xs">|</span>
          <span className="w-1.5 h-1.5 rounded-full bg-[#33ff33] animate-pulse" />
          <span className="hidden sm:inline text-[#33ff33] text-xs font-mono">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-1">
        {lines.map((line) => (
          <div key={line.id}>
            {line.type === 'banner' && (
              <pre className="text-[#1a8a1a] text-[10px] leading-tight mb-2 select-none">
                {line.text}
              </pre>
            )}
            {line.type === 'boot' && (
              <p className="text-[#1a6b1a] text-xs">
                <span className="text-[#33ff33]">[OK]</span> {line.text}
              </p>
            )}
            {line.type === 'input' && (
              <p>
                <span className="text-[#33ff33]">kim@portfolio:~$</span>{' '}
                <span className="text-[#e2e2f0]">{line.text}</span>
              </p>
            )}
            {line.type === 'ai' && (
              <div className="pl-4">
                {(line.source === 'ollama' || line.source === 'claude') && (
                  <div className="mb-0.5">
                    <span className="text-[#1a6b1a] text-[10px] font-mono border border-[#1a6b1a] px-1 rounded select-none">
                      AI
                    </span>
                  </div>
                )}
                <div className="text-[#33ff33] whitespace-pre-wrap leading-relaxed">
                  {line.text ? (
                    ...
                    <span className="inline-block w-2 h-4 bg-[#33ff33] animate-pulse" />
                  )}
                </div>
              </div>
            )}
            ...
            <div className="px-4 pt-2 pb-1 flex flex-col gap-0.5 border-b border-[#0a2a0a]">
              {slashSuggestions.map((c, i) => (
                <button
                  ...
                  className={[
                    'flex items-baseline gap-3 text-left px-1 rounded transition-colors',
                    i === activeIndex ? 'bg-[#0a2a0a]' : 'hover:bg-[#0a2a0a]',
                  ].join(' ')}
                >
                  <span
                    className={[
                      'text-xs font-mono w-28 shrink-0',
                      i === activeIndex ? 'text-[#33ff33]' : 'text-[#1a8a1a]',
                    ].join(' ')}
                  >
                    {c.cmd}
                  </span>
              ...
              <p className="text-[#1a4a1a] text-[10px] font-mono mt-0.5">
